...
async function createAnonymousViewLink(token, driveId, itemId) {
  ...
  // 기본 웹 링크(webUrl)는 브라우저로 열면 "쉐어포인트 미리보기 화면"이 뜨는 링크라,
  // <video>/<img> 태그에 그대로 쓰면 내용이 안 뜹니다. "download=1"을 붙이면
  // 파일 내용 자체를 바로 스트리밍/표시해주는 링크가 됩니다.
  const separator = webUrl.includes("?") ? "&" : "?";
  return `${webUrl}${separator}download=1`;
}
